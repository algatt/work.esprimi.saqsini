<template>
  <div class="flex flex-col justify-between w-full">
    ...
    <div class="w-full flex py-10 flex justify-between">
      ...
      ></button-basic>
      <button-basic :disabled="!isValid" @click="sendReminders"
        >Send Reminders<template v-slot:icon
...
export default {
  ...
  created() {
    ...
    this.contacts = this.emails.join(';')
  },
  methods: {
    sendReminders() {
      const list = {
        surveyCode: this.survey.code,
        reminderTimestamp: this.form.notificationTimestamp,
        reminderMessage: this.form.notificationMessage,
      ...
      this.$store
        .dispatch('invitations/sendReminder', list)
        .then(() => {
          this.$toasted.show('Reminders set succcessfully')
          this.$emit('cancel')
        })
        .catch(() => {
          this.$toasted.show('There was a problem sending the reminders')
        })
    },
  },
}
